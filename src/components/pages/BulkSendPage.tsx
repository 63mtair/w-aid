...
export default function BulkSendPage({ preselectedBeneficiaryIds = [] }: BulkSendPageProps) {
  ...
  React.useEffect(() => {
    if (preselectedBeneficiaryIds.length > 0) {
      ...
      logInfo(`تم تحديد ${preselectedBeneficiaryIds.length} مستفيد مسبقاً`, 'BulkSendPage');
    }
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        logInfo(`تم تحليل الملف: ${results.successful.length} جديد، ${results.updated.length} محدث، ${results.errors.length} خطأ`, 'BulkSendPage');
      } catch (error) {
  ...
  const handleConfirmImport = () => {
    ...
    logInfo(`تم استيراد ${importResults.successful.length + importResults.updated.length} مستفيد`, 'BulkSendPage');
  };
  ...
  const executeCreateTasks = () => {
    ...
    logInfo(`تم إنشاء ${newTasks.length} مهمة توزيع جماعية`, 'BulkSendPage');
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <ConfirmationModal
        ...
        onConfirm={executeCreateTasks}
        title="تأكيد إنشاء مهام التوزيع"
        message={`هل أنت متأكد من إنشاء ${selectedBeneficiaries.length} مهمة توزيع؟

سيتم إنشاء مهام للمستفيدين التالية:
...
هذا الإجراء لا يمكن التراجع عنه.`}
        confirmButtonText={`إنشاء ${selectedBeneficiaries.length} مهمة`}
      ...
      <Card className="bg-blue-50 border-blue-200">
        <div className="flex items-start space-x-3 space-x-reverse">
          ...
          <div>
            <h4 className="font-medium text-blue-800 mb-3">إرشادات الإرسال الجماعي</h4>
            <ul className="text-sm text-blue-700 space-y-2">
              <li className="flex items-start space-x-2 space-x-reverse">
                <CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                <span>اختر نوع الداعم أولاً (مؤسسة أو عائلة) لعرض القوالب المتاحة</span>
              </li>
              <li className="flex items-start space-x-2 space-x-reverse">
                <CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                <span>يمكن استيراد المستفيدين من ملف CSV أو إضافتهم يدوياً</span>
              </li>
              <li className="flex items-start space-x-2 space-x-reverse">
                <CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                <span>سيتم إنشاء مهمة توزيع منفصلة لكل مستفيد محدد</span>
              </li>
              <li className="flex items-start space-x-2 space-x-reverse">
                <AlertTriangle className="w-4 h-4 text-orange-600 mt-0.5 flex-shrink-0" />
                <span>تأكد من صحة بيانات المستفيدين قبل إنشاء المهام</span>
              </li>
            </ul>
          </div>
        </div>
